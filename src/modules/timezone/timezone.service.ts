...
export class TimezoneService {
    ...
    public async getByCountry3Alpha(code: string): Promise<Timezone[]> {
        if (code.length !== 3) throw new ValidationException(`Country code must be with 3 symbols`);
    ...
    public async getByCountry2Alpha(code: string): Promise<Timezone[]> {
        if (code.length !== 2) throw new ValidationException(`Country code must be with 2 symbols`);
