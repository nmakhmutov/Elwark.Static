...
export class ImageService {
    ...
    public getResolutionBySize = (width: number, height: number): ImageModel => {
        const orientation: ImageOrientation = width >= height ? 'landscape' : 'portrait';

        const images = RESOLUTIONS.filter((x) => x.orientation === orientation).sort((a, b) => b.pixels - a.pixels);
    ...
    ): ImageModel | undefined => RESOLUTIONS.find((x) => x.name === name && x.orientation === orientation);
}
