...
export class ImageWorker {
    ...
    constructor(private readonly loggerService: LoggerService, private readonly utils: UtilsService) {
        if (!existsSync(IMAGE_RANDOM_FOLDER)) mkdirSync(IMAGE_RANDOM_FOLDER, { recursive: true });

        readdir(IMAGE_RANDOM_FOLDER, async (_, files) => {
            let count = files.length;
            while (count <= MAX_IMAGES_COUNT) count += await this.loadImages();
        });
    }
    ...
    private loadImages = async (): Promise<number> => {
        ...
        const images = RESOLUTIONS.map((x) =>
            this.getRandomImage(x.width, x.height)
                .then((data) => {
                    writeFileSync(
                        join(
                            IMAGE_RANDOM_FOLDER,
                            getFilePrefix(x.name, x.orientation, `${this.utils.getRandomString(10)}.jpg`),
                        ),
                        data,
                    );
                    return true;
    ...
    private deleteImages = async (): Promise<number> => {
        ...
        const images = files
            .map((file) => resolve(IMAGE_RANDOM_FOLDER, file))
